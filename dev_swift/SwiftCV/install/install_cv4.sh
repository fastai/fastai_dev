...
cmake -D CMAKE_BUILD_TYPE=Release -D ENABLE_FAST_MATH=ON -D WITH_CSTRIPES=OFF -D WITH_PTHREADS_PF=OFF  -D OPENCV_GENERATE_PKGCONFIG=YES -D WITH_OPENCL=OFF -D WITH_QT=OFF  WITH_TBB=OFF -D WITH_OPENMP=OFF -D WITH_IPP=OFF -D BUILD_DOCS=OFF -D BUILD_PERF_TESTS=OFF -D BUILD_TESTS=OFF -D WITH_GDAL=ON -D WITH_XINE=ON -D ENABLE_PRECOMPILED_HEADERS=OFF ..
make -j4
sudo make install
sudo ldconfig
popd
