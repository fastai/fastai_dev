...
__all__ = ['chk', 'ifnone', 'noop', 'noops', 'range_of', 'is_listy', 'is_iter', 'listify', 'tuplify', 'tensor',
           'compose', 'uniqueify', 'setify', 'make_cross_image', 'mask2idxs', 'coll_repr', 'add_docs', 'docs',
           'ListContainer', 'opt_call']
...
class ListContainer():
    ...
    def __getitem__(self, idx):
        res = [self.items[i] for i in mask2idxs(idx)] if is_iter(idx) else self.items[idx]
        if is_iter(res) and not isinstance(res,ListContainer): res = ListContainer(res)
        return res
