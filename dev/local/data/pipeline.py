...
__all__ = ['Transform', 'XYTransform', 'Pipeline', 'TransformedList', 'Tuplify']
...
class Transform():
    ...
    def __call__(self, b, filt=None, **kwargs):
        return self._apply(self.encodes, b, **kwargs) if self._filt_match(filt) else o
    def decode  (self, b, filt=None, **kwargs):
        return self._apply(self.decodes, b, **kwargs) if self._filt_match(filt) else o
...
class XYTransform(Transform):
    "`Transform` that defaults `mask` to (True,True)"
    def __init__(self, encodes=None, is_tuple=None, **kwargs):
        super().__init__(encodes=encodes, mask=(True,True), is_tuple=is_tuple)
...
class TransformedList(GetAttr):
    ...
    def setup(self): self.tfm.setup(self)
    ...
    @classmethod
    def build(cls, items, tfms): return cls(items, Pipeline(tfms))

    _docs = dict(setup="Transform setup with self",
                 decode_at="Decoded item at `idx`",
                 show_at="Show item at `idx`",
                 subset="New `TransformedList` that only includes items at `idxs`")
