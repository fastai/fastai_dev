...
__all__ = ['get_func', 'Func', 'Sig', 'compose_tfms', 'batch_to_samples', 'mk_transform', 'Pipeline', 'TfmdList',
           'DataSource']
...
class TfmdList(L):
    ...
    def subset(self, i):
        res = self._new(self._gets(self.filts[i]))
        res.filt = i
        return res
...
class DataSource():
    ...
    def subsets(self): return map(self.subset, range_of(self.tls[0].filts))
    ...
    def subset(self, i):
        res = DataSource([])
        res.tls = [tl.subset(i) for tl in self.tls]
        return res
