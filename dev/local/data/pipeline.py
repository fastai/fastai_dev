...
__all__ = ['get_func', 'Func', 'Sig', 'SelfFunc', 'Self', 'compose_tfms', 'batch_to_samples', 'mk_transform',
           'Pipeline', 'TfmdList', 'TfmdList', 'TfmdDS']
...
class TfmdList():
    ...
    def __init__(self, items, tfms, do_setup=True):
        self.items = L(items)
        self._mk_pipeline(tfms.tfms if isinstance(tfms,TfmdList) else tfms, do_setup=do_setup)

    def _mk_pipeline(self, tfms, do_setup=True):
    ...
    # Below are delegating to `self.tfms`
    ...
    def show(self, *x, **kwargs): return self.tfms.show(*x, **kwargs)
