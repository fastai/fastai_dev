...
__all__ = ['Transform', 'Pipeline', 'TransformedList', 'Pipelines']
...
class Pipelines(Transform):
    "Create a `Pipeline` for each tfm in `tfms`. Generally used inside a `TransformedList`"
    ...
    def build(cls, tfms, final_tfms=None):
        "TransformedList over `items` with `tfms` `Pipelines` as first tfm optionally followed by any `xtra` tfms"
