...
__all__ = ['show_batch', 'show_results', 'TfmdDL', 'DataBunch', 'IterableDataBunch', 'FilteredBase', 'TfmdList',
           'TfmdIterator', 'decode_at', 'show_at', 'DataSource', 'IterableDataSource', 'test_set', 'test_dl']
...
#Cell
@docs
class IterableDataBunch(GetAttr):
    "Basic wrapper around several `DataLoader`s."
    _default='train_dl'
...
#Cell
class TfmdIterator(GetAttr): # TODO docs
    "A `Pipeline` of `tfms` applied to an iterator `it`"
    _default='tfms'
    def __init__(self, it, tfms, use_list=None, as_item=True, split_idx=None):
        super().__init__()
        store_attr(self, "it")
        self.tfms = Pipeline(tfms, as_item=as_item, split_idx=split_idx)

    def _after_item(self, o): return self.tfms(o)
    def __repr__(self): return f"{self.__class__.__name__}: {self.it}\ntfms - {self.tfms.fs}"
    def show(self, o, **kwargs): return self.tfms.show(o, **kwargs)
    def decode(self, x, **kwargs): return self.tfms.decode(x, **kwargs)
    def __call__(self, x, **kwargs): return self.tfms.__call__(x, **kwargs)
