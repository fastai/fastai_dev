...
__all__ = ['ShowBatch', 'default_show_batch', 'subplots', 'image_show_batch', 'TfmdDL', 'DataBunch', 'FilteredBase',
           'TfmdList', 'decode_at', 'show_at', 'DataSource', 'test_set', 'test_dl']
...
ShowBatch.add(default_show_batch)
...
#Cell
def image_show_batch(x:TensorImage, y, db, ctxs=None, max_n=10, rows=None, cols=None, figsize=None, **kwargs):
    if ctxs is None:
        n_samples = min(len(db), max_n)
...
ShowBatch.add(image_show_batch)
...
class TfmdDL(DataLoader):
    ...
    def _show_batch(self, b, ctxs, max_n=10, **kwargs):
        ...
        x,y = b[:self.n_inp],b[self.n_inp:]
        ctxs = ShowBatch(detuplify(x), detuplify(y) if len(y) > 0 else None, db, max_n=max_n, **kwargs)
