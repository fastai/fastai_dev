...
__all__ = ['default_show_batch', 'show_batch', 'subplots', 'show_batch', 'TfmdDL', 'DataBunch', 'FilteredBase',
...
#Cell
def default_show_batch(x, y, db, ctxs=None, max_n=10, **kwargs):
...
#Cell
@typedispatch
def show_batch(x:TensorImage, y, db, ctxs=None, max_n=10, rows=None, cols=None, figsize=None, **kwargs):
    if ctxs is None:
...
class TfmdDL(DataLoader):
    ...
    def _show_batch(self, b, ctxs, max_n=10, **kwargs):
        ...
        ctxs = show_batch(detuplify(x), detuplify(y) if len(y) > 0 else None, db, max_n=max_n, **kwargs)
