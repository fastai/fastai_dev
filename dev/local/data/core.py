...
__all__ = ['show_batch', 'show_results', 'TfmdDL', 'DataBunchBase', 'DataBunch', 'FilteredBase', 'TfmdIterableBase',
           'TfmdList', 'TfmdIterator', 'decode_at', 'show_at', 'DataSource', 'IterableDataSource', 'test_set',
           'test_dl']
...
class TfmdDL(DataLoader):
    ...
    def _one_pass(self):
        its = self.after_batch(self.do_batch([self.one_item()]))
...
#Cell
class DataBunchBase(GetAttr):
    "Base for DataBunch class, makes no assumption that the dataset is indexable"
    _default='train_dl'
...
class TfmdIterableBase(GetAttr):
    "Base class for TfmdList and TfmdIterator"
    _default='tfms'
    def __init__(self, tfms, do_setup=True, as_item=True, split_idx=None, train_setup=True, **kwargs):
    ...
    def setup(self, train_setup=True): self.tfms.setup(self)
    # TODO necessary for passing up the call to _new ?
    #def _new(self, *args, **kwargs): return super()._new(*args, **kwargs)

class TfmdList(FilteredBase, TfmdIterableBase, L):
    "A `Pipeline` of `tfms` applied to a collection of `items`"
    def __init__(self, items, tfms, use_list=None, splits=None, **kwargs):
        self.splits = L([slice(None),[]] if splits is None else splits).map(mask2idxs)
        super().__init__(items=items, tfms=tfms, use_list=use_list, **kwargs)
...
class TfmdIterator(TfmdIterableBase): # TODO docs
    "A `Pipeline` of `tfms` applied to an iterator `items`"
...
class IterableDataSource():
    # TODO docs
    def __init__(self, items=None, tfms=None, tts=None, n_inp=None, dl_type=None, **kwargs):
        self.tts = L(tts if tts else [TfmdIterator(items, t, **kwargs) for t in L(ifnone(tfms,[None]))])
        self.n_inp = (1 if len(self.tts)==1 else len(self.tts)-1) if n_inp is None else n_inp

    def __iter__(self):
        yield from zip(*[tt for tt in self.tts])
    def __getattr__(self,k): return gather_attrs(self, k, 'tts')
    def __dir__(self): return super().__dir__() + gather_attr_names(self, 'tts')
    def __repr__(self): return coll_repr(self)
    def decode(self, o, full=True): return tuple(tl.decode(o_, full=full) for o_,tl in zip(o,tuplify(self.tts, match=o)))
    @property
    def split_idx(self): return self.tts[0].tfms.split_idx

    def show(self, o, ctx=None, **kwargs):
        for o_,tl in zip(o,self.tts): ctx = tl.show(o_, ctx=ctx, **kwargs)
        return ctx

    @contextmanager
    def set_split_idx(self, i):
        old_split_idx = self.split_idx
        for tl in self.tts: tl.tfms.split_idx = i
        yield self
        for tl in self.tts: tl.tfms.split_idx = old_split_idx
