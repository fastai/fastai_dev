...
#Cell
def radam_step(p, lr, mom, step, sqr_mom, grad_avg, sqr_avg, eps, thresh, **kwargs):
...
radam_step._defaults = dict(eps=1e-5, thresh=5)

#Cell
def RAdam(params, lr, mom=0.9, sqr_mom=0.99, eps=1e-5, thresh=5, wd=0., decouple_wd=True):
    "A `Optimizer` for Adam with `lr`, `mom`, `sqr_mom`, `eps` and `params`"
    steppers = [weight_decay] if decouple_wd else [l2_reg]
    steppers.append(radam_step)
    stats = [partial(average_grad, dampening=True), average_sqr_grad, step_stat]
    return Optimizer(params, steppers, stats=stats, lr=lr, mom=mom, sqr_mom=sqr_mom, eps=eps, thresh=thresh, wd=wd)
