...
__all__ = ['ProcessPoolExecutor', 'parallel', 'ProcessFunc', 'parallel_by_batch', 'UNK', 'PAD', 'BOS', 'EOS', 'FLD',
           'TK_REP', 'TK_WREP', 'TK_UP', 'TK_MAJ', 'spec_add_spaces', 'rm_useless_spaces', 'replace_rep',
...
class ProcessFunc():
    ...
    def __call__(self, batch): return (b for b in batch)

def _wrap_process_func(pfunc_cls, batch, send_end, *args, start_idx=None, **kwargs):
    f = pfunc_cls(*args, **kwargs)
    for i,b in enumerate(f(batch)):
        send_end.send(b if start_idx is None else (start_idx+i,b))
