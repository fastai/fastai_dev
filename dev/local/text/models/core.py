...
__all__ = ['LinearDecoder', 'SequentialRNN', 'get_language_model', 'MultiBatchEncoder', 'masked_concat_pool',
...
class MultiBatchEncoder(Module):
    ...
    def __init__(self, bptt, max_len, module, pad_idx=1): store_attr(self, 'bptt,max_len,module,pad_idx')

    def _concat(self, arrs): return [torch.cat([l[si] for l in arrs], dim=1) for si in range_of(arrs[0])]
    ...
    def forward(self, input):
        ...
        for i in range(0, sl, self.bptt):
            ...
            if i>=(sl-self.max_len):
                ...
                outputs.append(o)
        return self._concat(raw_outputs),self._concat(outputs),torch.cat(masks,dim=1)
...
#Cell
def get_text_classifier(arch, vocab_sz, n_class, bptt=72, max_len=20*72, config=None, drop_mult=1., lin_ftrs=None,
                        ps=None, pad_idx=1):
