...
class TabularPreprocessor():
    ...
    def __call__(self, df, trn_idx=None):
        ...
        if trn_idx is None:
            ...
            for p in procs:
                p_ = p(cat_names=self.cat_names + self.cat_y if p==Categorify else self.cat_names, cont_names=self.cont_names)
                p_.setup(df, trn_idx=trn_idx)
                p_(df)
                if p!= Categorify: self.cat_names,self.cont_names = p_.cat_names,p_.cont_names
                self.procs.append(p_)
            self.classes = {n:'#na#'+L(df[n].cat.categories, use_list=True) for n,c in df[self.cat_names + self.cat_y].items()}
            for p in self.procs:
...
def process_df(df, splits, procs, cat_names=None, cont_names=None, cat_y=None, inplace=True):
    ...
    proc = TabularPreprocessor(procs, cat_names, cont_names, cat_y, inplace=inplace)
